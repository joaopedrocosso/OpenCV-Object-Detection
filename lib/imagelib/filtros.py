...
def selecionandoContornos(frame,conf):
    caixas = []
    listaContornos,_ = cv2.findContours(frame,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
...
def verificarMudancaFundo(frameAnterior,frameAtual,tempoUltimoMovimento, minMovimentoMudanca=430):
    ...
    if nMatches<=minMovimentoMudanca: # se mudou, atualiza o tempo de analise ativa (430 eh um valor temporario)
        return time.time() 
    else:
